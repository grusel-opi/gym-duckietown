...
class PurePursuitPolicy:
    ...
    def predict(self, observation=None) -> np.ndarray:
        """
        Parameters
        ----------
        observation : image
        ...
        omega = -1 * dot
        # range of dot is just -pi/2 and pi/2 and will be multiplied
        # later by a gain adjustable if we are testing on a hardware or not
        velocity = self.ref_velocity * velocity_scale
        if velocity_slow_down < 0.2:
            velocity = 0
            omega = 0

        action = np.array([velocity, omega])
